   ...
   static class PingPongThreadSema extends PingPongThread
    {
        ...
        void acquire()
        {
            mSemas[FIRST_SEMA].acquireUninterruptibly(); //获得锁，  将 firstSema 的信号量变为 0。。。 排斥其他线程进入
        }
        ...
        void release()
        {
            mSemas[SECOND_SEMA].release();  //将 secondSema 信号量变为 1  。。告诉领一个类型的线程可以进入了。。
        }
    }
    ...
    static class PingPongThreadMonObj extends PingPongThread 
    {
        ...
        PingPongThreadMonObj(String stringToPrint,//-->ping or pong
